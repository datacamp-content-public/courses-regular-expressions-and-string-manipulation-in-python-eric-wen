...
`@part1`
In addition to `re.match()` Python's regular expression library also provides `re.search()`.

While `re.match()` begins its attempt to "match" from the beginning of each string, instead `re.search()` tries to "search" for the pattern **_anywhere_** in the target string.

In this lesson, we will distinguish between `re.match()` vs. `re.search()`.

We will try to match `Alan` and `Aolani`, entries in our familiar list of names:

     `li_names = ['Adele', 'Alan', 'Ana-Maria', 'Aolani', 'Arlene']`

We observe that both contain the lower case letters `an`.
