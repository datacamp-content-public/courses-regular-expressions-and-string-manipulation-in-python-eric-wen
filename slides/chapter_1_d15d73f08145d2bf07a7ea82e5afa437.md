...
`@script`
We typically use a `for` loop to traverse the list element-by-element, with typical output shown on the lower right.
...
`@script`
In the previous lesson, `re.match()` would have been introduced, as well as the wildcard `.` which matches any single character except newline `\n` and quantifiers.  One of the examples would have covered the regular expression show in this slide, which would be carried-over into this lesson.
...
`@script`
Here we have the regular expression that accomplishes the task with regular expression matching.


---
## `re.compile(r'.*an')`  with `re.search()`
...
`@script`
Here we have the same regular expression, however in this case we use the `re.search()` method and find that we basically have the same behavior as before.  This shows that in some cases both `re.match()` and `re.search()` yield the same match result.  In order to understand the difference between the two methods, we need to use a different regular expression.
...
`@script`
Now, let us use the simple literal regular expression `an` with both the `re.match()` and `re.search()` methods.  Here, you can see that `re.match()` produces no match result because there are no names that begin with `an`.


---
## `re.compile(r'an', re.IGNORECASE)` with `re.match()`
...
`@script`
You may notice that `Ana-Maria` begins with a capital `A` followed by `n`.  Can the regular expression be modified to match this name?  Yes, it turns-out that one can enter a `RegexFlag`, formerly known as a "Compilation Flag" to ignore the case, as in uppercase and lowercase, in the target string.  As you can see, now, the same regular expression `an` and the `re.IGNORECASE` `RegexFlag` can match the name `Ana-Maria`.
...
`@script`
Now, we return to the simple regular expression, `an` but this time use `re.search()`.  You can see that


---
## `re.compile(r'an', re.IGNORECASE)` with `re.search()`
...
`@script`
We can return to the regular expression with the `RegexFlag` set to the value `re.IGNORECASE`, which can be abbreviated `re.I`.  You can see that the result now includes three names along the coast.
...
`@script`
Now, it's your turn to get some practice writing regular expressions. &#x2713;
