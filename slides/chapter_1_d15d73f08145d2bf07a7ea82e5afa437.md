...
The difference is that `re.match()` begins its attempt to "match" the regular expression pattern from the beginning of each string, while `re.search()` can "search" for the regular expression pattern **_anywhere_** within the target string.
